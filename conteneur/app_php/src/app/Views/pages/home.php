...
<section class="relative bg-cream/30 pt-20 pb-32 overflow-hidden">
    ...
    <div
        class="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-white to-transparent opacity-50 pointer-events-none">
    </div>

    <div class="relative max-w-[1600px] mx-auto px-[5%] text-center pt-16">
        <span
            class="inline-block animate-fadeIn text-accent font-bold uppercase tracking-[0.2em] text-xs md:text-sm mb-6 bg-accent/5 px-4 py-2 rounded-full">
...
<section id="featured" class="max-w-[1600px] mx-auto px-[5%] py-24">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-16 border-b border-border-light pb-6">
        ...
        <a href="<?= base_url('catalog') ?>"
            class="group font-bold text-accent flex items-center gap-2 hover:text-primary transition-colors py-2">
            Tout voir
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <?php else: ?>
            <div class="col-span-full py-20 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
                ...
                <a href="<?= base_url('catalog') ?>" class="inline-block mt-4 text-accent font-bold hover:underline">Voir
                    tout le catalogue</a>
            </div>
        <?php endif; ?>
    </div>
