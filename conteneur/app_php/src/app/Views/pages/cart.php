        ...
        <div class="mb-10">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <a href="<?= base_url('cart/clear') ?>"
                    ...
                    class="text-xs font-bold text-red-500 hover:text-red-700 uppercase tracking-widest flex items-center gap-1.5 transition-colors group py-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor" class="w-4 h-4 transition-transform group-hover:scale-110">
                                ...
                                <form action="<?= base_url('cart/update') ?>" method="post">
                                    ...
                                    <div class="relative">
                                        <select name="quantity" onchange="this.form.submit()"
                                            class="appearance-none w-20 bg-gray-50 border border-gray-200 text-gray-700 py-2 pl-3 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent text-sm font-bold cursor-pointer hover:bg-white transition-colors">
                                            <?php
                        ...
                        <?php foreach ($items as $item): ?>
                            <div class="flex justify-between text-sm">
                                <div class="flex flex-col flex-1 pr-4">
                                    <span
                                        class="font-medium text-gray-800 line-clamp-1"><?= esc($item->getProductName()) ?></span>
                                    <span class="text-xs text-gray-500"><?= $item->quantity ?> x
                                        <?= $item->getFormattedUnitPrice() ?></span>
                                </div>
                                <span
                                    class="font-medium text-gray-700 whitespace-nowrap"><?= $item->getFormattedSubtotal() ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
    ...
    <?php else: ?>
        <div class="flex flex-col justify-center items-center min-h-[50vh] text-center px-4">
            <div class="bg-gray-50 p-6 rounded-full mb-6">
                <img src="https://cdn-icons-png.flaticon.com/512/11329/11329060.png" alt="Panier vide"
                    class="w-16 opacity-20">
            </div>
            <h2 class="font-serif text-3xl text-gray-900 mb-3">Votre panier est vide</h2>
            <p class="text-gray-500 mb-8 max-w-md mx-auto leading-relaxed">Il semble que vous n'ayez pas encore trouvé votre
                bonheur. Explorez notre catalogue pour dénicher des merveilles.</p>
            <?= view('partials/black_button', [
