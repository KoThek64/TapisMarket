...
<div class="max-w-4xl mx-auto grid md:grid-cols-3 gap-8">
    ...
    <div class="md:col-span-3 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 p-8 ring-1 ring-gray-100">
            <h3 class="font-serif font-bold text-2xl text-primary mb-8 border-b-2 border-gray-100 pb-4">Informations du
                compte</h3>
            ...
            <form action="<?= site_url('client/profile/update') ?>" method="post" class="space-y-6">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="birth_date"
                            value="<?= esc($user->birth_date ? $user->birth_date->format('Y-m-d') : '') ?>"
                            class="w-full rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white focus:border-accent focus:ring-accent transition-all duration-300 p-3 shadow-sm">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input type="email" name="email" value="<?= esc($user->email) ?>"
                            class="w-full rounded-xl border-gray-200 bg-gray-100 text-gray-500 cursor-not-allowed p-3 shadow-sm"
                            readonly title="Contactez le support pour changer l'email">
                ...
                <div class="pt-6 border-t border-gray-100 flex justify-end">
                    <button type="submit"
                        class="px-8 py-3 bg-primary text-white font-bold rounded-xl hover:bg-gray-800 transition shadow-lg shadow-gray-200 transform hover:-translate-y-0.5">
        ...
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 p-8 ring-1 ring-gray-100">
            <div class="flex justify-between items-center mb-8 border-b-2 border-gray-100 pb-4">
                ...
                <a href="<?= site_url('client/addresses/new') ?>"
                    ...
                    <?php foreach ($addresses as $addr): ?>
                        <div class="p-4 rounded-xl border border-border hover:border-accent transition group relative">
                            ...
                            <div class="text-sm text-gray-600 space-y-0.5">
                                ...
                                <?php if (!empty($addr->contact_phone)): ?>
                                    <p class="text-xs text-muted mt-2">Tel: <?= esc($addr->contact_phone) ?></p><?php endif; ?>
                            </div>
                            <div
                                class="mt-3 pt-3 border-t border-dashed border-border flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="<?= site_url('client/addresses/' . $addr->id . '/edit') ?>"
                                    class="text-xs font-bold text-muted hover:text-accent">Modifier</a>
                                <form action="<?= site_url('client/addresses/' . $addr->id) ?>" method="post"
                                    onsubmit="return confirm('Supprimer cette adresse ?')" class="inline">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button type="submit"
                                        class="text-xs font-bold text-red-400 hover:text-red-600">Supprimer</button>
                                </form>
                            </div>
                        </div>
                    <?php endforeach; ?>
        ...
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 p-8 ring-1 ring-gray-100">
            <h3 class="font-serif font-bold text-2xl text-primary mb-6 border-b-2 border-gray-100 pb-4">Mes données
                personnelles</h3>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <a href="<?= site_url('client/profile/export') ?>"
                    class="inline-flex items-center gap-2 px-5 py-2.5 border border-gray-300 rounded-xl text-gray-700 font-bold text-sm hover:bg-gray-50 hover:border-gray-400 transition-all shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
