    ...
    <div class="w-full max-auto max-w-md bg-white p-8 rounded-custom shadow-md-custom border border-border-custom my-8">
        ...
        <form action="<?= base_url('auth/register') ?>" method="post" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="firstname" id="firstname"
                        ...
                        placeholder="Jean" value="<?= old('firstname') ?>" required>
                </div>
                <div>
            ...
            <div>
                ...
                <div class="flex gap-4 p-1 bg-gray-50 rounded-xl border border-border-custom">
                    <label
                        class="flex items-center gap-2 cursor-pointer flex-1 justify-center py-2 rounded-lg hover:bg-white transition-all shadow-sm">
                        <input type="radio" name="role" value="client" id="role_client" class="accent-accent w-4 h-4"
                            <?php if (old('role') !== 'vendeur')
                                echo 'checked'; ?>>
                        <span class="font-sans text-sm font-medium">Client</span>
                    </label>
                    <label
                        class="flex items-center gap-2 cursor-pointer flex-1 justify-center py-2 rounded-lg hover:bg-white transition-all shadow-sm">
                        <input type="radio" name="role" value="vendeur" id="role_vendeur" class="accent-accent w-4 h-4"
                            <?php if (old('role') === 'vendeur')
                                echo 'checked'; ?>>
            ...
            <div id="seller-fields"
                class="space-y-4 border-l-4 border-accent pl-4 py-2 <?php if (old('role') !== 'vendeur')
                    echo 'hidden'; ?>">
                <h3 class="font-serif text-lg text-accent font-semibold">Informations Vendeur</h3>
                <div>
                    <label for="shop_name" class="block text-sm font-medium text-primary mb-1 italic">Nom de la
                        boutique</label>
                    <input type="text" name="shop_name" id="shop_name"
                        class="w-full px-4 py-3 rounded-xl border border-border-custom focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all font-sans bg-white"
                        placeholder="Ma Boutique de Tapis" value="<?= old('shop_name') ?>">
                </div>
                <div>
                    ...
                    <input type="text" name="siret" id="siret"
                        class="w-full px-4 py-3 rounded-xl border border-border-custom focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all font-sans bg-white"
                        placeholder="12345678900012" value="<?= old('siret') ?>" pattern="[0-9]{14}" minlength="14"
                        maxlength="14" title="Le numéro SIRET doit comporter exactement 14 chiffres.">
                </div>
                <div>
                    <label for="shop_description"
                        class="block text-sm font-medium text-primary mb-1 italic">Description</label>
                    <textarea name="shop_description" id="shop_description"
                        class="w-full px-4 py-3 rounded-xl border border-border-custom focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all font-sans bg-white"
                        placeholder="Spécialiste des tapis d'orient..."
            ...
            <div>
                ...
                <input type="email" name="email" id="email"
                    class="w-full px-4 py-3 rounded-xl border border-border-custom focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all font-sans"
                    placeholder="exemple@mail.com" value="<?= old('email') ?>" required>
            </div>

            <div>
                <label for="phone" class="block text-sm font-medium text-primary mb-1 italic">Numéron de
                    téléphone</label>
            ...
            <div>
                <label for="password_confirm" class="block text-sm font-medium text-primary mb-1 italic">Confirmer le
                    mot de passe</label>
        ...
        <div class="mt-8 pt-6 border-t border-border-custom text-center">
            <p class="text-sm text-muted">
                ...
                <a href="<?= base_url('auth/login') ?>" class="text-accent font-semibold hover:underline">Se
                    connecter</a>
            </p>
        </div>
    </div>
