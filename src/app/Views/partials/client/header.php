...
<header class="h-24 px-8 flex justify-between items-center bg-white/80 backdrop-blur-md border-b border-border sticky top-0 z-20">
    ...
    <div class="flex items-center gap-4">
        ...
        <a href="<?= site_url('/') ?>" class="flex items-center gap-2 px-4 py-2 border border-border rounded-xl hover:bg-white hover:shadow-sm transition text-sm font-bold text-gray-700 bg-gray-50/50">
        ...
        <div class="w-10 h-10 rounded-full bg-accent text-white flex items-center justify-center font-serif font-bold text-lg shadow-lg shadow-accent/20 cursor-default" title="<?= esc($firstname) ?>">
            <?= esc($initials) ?>
        </div>
    </div>
</header>
