...
<aside class="w-72 bg-white border-r border-border flex flex-col shadow-xl z-30 hidden lg:flex">
    ...
    <nav class="flex-1 px-4 py-8 space-y-2">
        ...
        <a href="<?= site_url('client/dashboard') ?>" class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all <?= getMenuClass($segment === 'dashboard') ?>">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
            Tableau de bord
        </a>

        <a href="<?= site_url('client/orders') ?>" class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all <?= getMenuClass($segment === 'orders') ?>">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
            Mes Commandes
        </a>

        <a href="<?= site_url('client/reviews') ?>" class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all <?= getMenuClass($segment === 'reviews') ?>">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path></svg>
            Mes Avis
        </a>

        <a href="<?= site_url('client/profile') ?>" class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all <?= getMenuClass($segment === 'profile') ?>">
