<article
    class="group bg-white border border-transparent rounded-2xl overflow-hidden transition-all duration-300 flex flex-col relative hover:-translate-y-2 hover:shadow-xl">
    <a href="<?= base_url('product/' . $product->alias) ?>"
        class="relative block pt-[130%] overflow-hidden bg-gray-100 rounded-t-2xl">
        <?php
        ...
        ?>
        <img src="<?= esc($imgSrc) ?>" alt="<?= esc($product->title) ?>"
            ...
            <div class="cart-action">
                ...
                <?php if ($isAdminOrSeller): ?>
                    <button type="button" disabled
                        class="px-5 py-2.5 text-sm inline-flex items-center justify-center gap-2 bg-gray-100 text-gray-400 font-bold rounded cursor-not-allowed border border-gray-200"
...
<?php if (!isset($GLOBALS['cart_script_included'])):
    $GLOBALS['cart_script_included'] = true; ?>
    <script>
