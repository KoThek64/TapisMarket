    ...
    <div class="bg-white rounded-2xl shadow-md border-2 border-orange-100 overflow-hidden relative mb-8">
        ...
        <div class="p-8 grid gap-6">
            <?php foreach ($pendingProducts as $prod): ?>
                <div
                    class="flex flex-col xl:flex-row items-start gap-6 p-4 border border-border rounded-xl hover:shadow-lg transition-all bg-white group">
                    ...
                    <div class="w-24 h-24 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden border border-border relative group-hover:border-accent transition-colors cursor-pointer"
                        ...
                        <?php else: ?>
                            <div class="w-full h-full flex items-center justify-center text-gray-300"><svg class="w-8 h-8"
                    ...
                    <div class="flex-1 w-full pt-1">
                        <div class="flex justify-between items-start">
                            ...
                            <button onclick='openModal(<?= json_encode($prod, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                                class="text-gray-400 hover:text-accent p-1" title="View details">
                        ...
                        <div class="flex flex-wrap gap-4 mt-2 text-sm text-muted">
                            <span class="flex items-center gap-1"> <strong
                                    class="text-primary"><?= esc($prod->shop_name) ?></strong></span>
                            <span class="flex items-center gap-1"> <strong
                                    class="text-accent"><?= number_format($prod->price, 2) ?> €</strong></span>
                            <span class="flex items-center gap-1"> <?= date('d/m/Y', strtotime($prod->created_at)) ?></span>
                        </div>
                        <p class="text-sm text-gray-500 mt-2 line-clamp-1 italic"><?= esc($prod->short_description ?? '...') ?>
                        </p>
                    </div>

                    <div class="flex flex-col sm:flex-row items-center gap-2 w-full xl:w-auto mt-2 xl:mt-0">
                        <a href="<?= site_url('admin/products/approve/' . $prod->id) ?>"
                            class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-bold shadow-md hover:shadow-lg transition w-full sm:w-auto text-center text-sm">Valider</a>
                        <form action="<?= site_url('admin/products/reject/' . $prod->id) ?>" method="post"
                            class="flex gap-2 w-full sm:w-auto">
                            <?= csrf_field() ?>
                            <input type="text" name="reason"
                                class="flex-1 px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-red-200 outline-none"
                                placeholder="Raison...">
                            <button type="submit"
                                class="bg-white text-red-600 border border-red-200 hover:bg-red-50 px-4 py-2 rounded-lg font-bold transition text-sm">Refuser</button>
                        </form>
                    </div>
                </div>
            <?php endforeach; ?>
                    ...
                    <tr class="hover:bg-cream transition-colors">
                        <td class="px-8 py-4">
                            <div class="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden border border-border cursor-pointer"
                                ...
                                <?php if (!empty($prod->image)): ?>
                                    <img src="<?= $prod->getImageUrl($prod->image) ?>" class="w-full h-full object-cover">
                                <?php else: ?>
                                    <div class="w-full h-full flex items-center justify-center text-gray-300"><svg
                        ...
                        <td class="px-8 py-4">
                            <?php if ($prod->product_status === STATUS_APPROVED): ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-green-100 text-green-800 border border-green-200 uppercase tracking-wide">
                                    Publié
                                </span>
                            <?php elseif ($prod->product_status === STATUS_REFUSED): ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-red-100 text-red-800 border border-red-200 uppercase tracking-wide">
                                    Rejeté
                                </span>
                            <?php else: ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-orange-100 text-orange-800 border border-orange-200 uppercase tracking-wide">
                                    En attente
                                </span>
                            <?php endif; ?>
                        </td>
