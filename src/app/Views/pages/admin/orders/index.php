...
<div class="bg-white rounded-2xl shadow-sm border border-border overflow-hidden flex flex-col">
    ...
    <div
        class="px-8 py-6 border-b border-border bg-gray-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-4">
            <h2 class="font-serif font-bold text-xl text-primary">Transactions</h2>
            <span
                class="bg-primary text-white text-[10px] px-3 py-1 rounded-full uppercase font-bold tracking-tighter shadow-md">
            ...
            <tbody class="divide-y divide-border text-sm">
                <?php foreach ($orders as $order): ?>
                    <tr class="hover:bg-cream transition-colors group">
                        <td class="px-8 py-5">
                            <span
                                class="font-mono text-xs font-bold text-accent px-2 py-1 bg-orange-50 border border-orange-100 rounded">#<?= esc($order->reference) ?></span>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-gray-500 border border-gray-200">
                                    <?= strtoupper(substr($order->lastname, 0, 1)) ?>
                                </div>
                                <span class="text-sm font-bold text-primary italic font-serif"><?= esc($order->firstname) ?>
                                    <?= esc($order->lastname) ?></span>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-5">
                            ...
                            ?>
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border <?= $color ?>">
                                <?= esc($statusText) ?>
                            </span>
                        </td>
                        ...
                        <td class="px-8 py-5 text-right">
                            <span
                                class="text-[10px] font-bold bg-gray-50 px-3 py-1.5 rounded border border-border text-muted uppercase tracking-tighter">
