...
<div class="flex items-center gap-4">
    <a href="<?= site_url('admin/orders') ?>"
        class="w-8 h-8 rounded-full border border-border flex items-center justify-center text-muted hover:bg-primary hover:text-white transition">
        ←
    </a>
    <div>
        <h1 class="font-serif text-2xl font-bold text-primary">Commande <span
                class="text-accent italic">#<?= esc($order->reference) ?></span></h1>
    ...
    <div class="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-border">
                <h2 class="text-xs font-bold uppercase text-muted mb-6 tracking-widest border-b border-border pb-2">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="font-serif text-xl font-bold text-primary"><?= esc($order->firstname) ?>
                            <?= esc($order->lastname) ?></p>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-border">
                ...
                <div class="flex gap-4">
                    <div
                        class="w-12 h-12 rounded-full bg-orange-50 text-accent flex items-center justify-center text-xl">
                        📍
                    </div>
                    <div>
                        ...
                        <div
                            class="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-gray-50 rounded-lg border border-border text-xs font-bold text-muted uppercase">
            ...
            <div class="bg-primary text-white p-8 rounded-2xl shadow-xl sticky top-8">
                ...
                <div class="space-y-4 mb-8">
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span
                            class="font-bold"><?= number_format($order->total_ttc - $order->shipping_fees, 2, ',', ' ') ?>
                            €</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="opacity-70 font-medium">Frais de port</span>
                        <span class="text-green-400 font-bold">+
                            <?= number_format($order->shipping_fees, 2, ',', ' ') ?> €</span>
                    </div>
                </div>
