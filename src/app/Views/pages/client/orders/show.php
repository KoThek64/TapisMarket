...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
            ...
            <div class="divide-y divide-border">
                ...
                <?php else: ?>
                    <?php foreach ($items as $item): ?>
                        <div
                            class="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6 group hover:bg-gray-50/30 transition-colors">
                            <!-- Image -->
                            <div
                                class="w-20 h-20 bg-gray-50 rounded-xl border border-border flex items-center justify-center shrink-0 overflow-hidden relative">
                                <img src="<?= $item->getImage() ?>" alt="<?= esc($item->title) ?>"
                            ...
                            <?php if (in_array($order->status, ['DELIVERED', 'COMPLETED'])): ?>
                                <div class="ml-2">
                                    <a href="<?= site_url('client/reviews/' . $item->product_id . '/edit/') ?>"
                                        class="p-2.5 text-accent hover:bg-accent/10 hover:text-accent-hover rounded-xl transition flex flex-col items-center gap-1 group/btn"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-border p-6 flex flex-col sm:flex-row gap-6">
            <div class="flex-1">
                <h3 class="font-bold text-sm uppercase text-muted tracking-wider mb-4 border-b border-border pb-2">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-border p-6 sticky top-8">
            ...
            <div class="space-y-4 text-sm">
                <div class="flex justify-between items-center">
                    ...
                    <span
                        class="font-bold text-primary"><?= number_format($order->total_ttc - ($order->shipping_fees ?? 0), 2) ?>
                        €</span>
                </div>
                <div class="flex justify-between items-center">
            ...
            <div class="mt-6 pt-6 border-t border-border border-dashed">
                <div class="flex justify-between items-end">
                    ...
                    <div class="text-right">
                        <span
                            class="block font-serif font-bold text-2xl text-accent"><?= number_format($order->total_ttc, 2) ?>
                            €</span>
            ...
            <div class="mt-6 pt-6 border-t border-border flex justify-center">
                <button onclick="window.print()"
                    class="flex items-center justify-center gap-2 px-6 py-3 bg-gray-50 text-primary font-bold rounded-xl text-sm hover:bg-gray-100 transition border border-border w-full">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
