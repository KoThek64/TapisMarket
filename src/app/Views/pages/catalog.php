...
<?= $this->section("content") ?>
    ...
    <!-- Main Content Layout -->
    <form action="<?= base_url('catalog') ?>" method="get" class="max-w-[1600px] mx-auto px-[5%] py-8 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10">
        ...
        <aside class="hidden lg:block sticky top-24 max-h-[calc(100vh-150px)] overflow-y-auto pr-2">
            ...
            <div class="border-b border-border-light py-6">
                ...
                <button type="button" class="w-full flex justify-between items-center mb-4 cursor-pointer focus:outline-none group" onclick="toggleFilter(this, 'categories')">
                    <span class="font-semibold text-[15px]">Catégories</span>
                    <span class="text-xs transition-transform duration-200" style="transform: rotate(<?= $isCatOpen ? '0deg' : '-90deg' ?>);">▼</span>
                </button>
                    ...
                    <?php if (!empty($categories)): ?>
                        <?php foreach ($categories as $category): ?>
                            <label class="flex items-center gap-3 text-sm cursor-pointer hover:text-accent transition-colors">
                                <input type="checkbox" name="cat[]" value="<?= esc($category->name) ?>" class="w-4 h-4 accent-primary" 
                                    <?= in_array($category->name, $selectedCategories ?? []) ? 'checked' : '' ?> onchange="setActiveSection('categories'); this.form.submit()"> <?= esc($category->name) ?>
                            </label>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <div class="border-b border-border-light py-6">
                ...
                <button type="button" class="w-full flex justify-between items-center mb-4 cursor-pointer focus:outline-none group" onclick="toggleFilter(this, 'materials')">
                    <span class="font-semibold text-[15px]">Matière</span>
                    <span class="text-xs transition-transform duration-200" style="transform: rotate(<?= $isMatOpen ? '0deg' : '-90deg' ?>);">▼</span>
                </button>
                <div class="flex flex-col gap-3 <?= $isMatOpen ? '' : 'hidden' ?>">
                    <?php if (!empty($materials)): ?>
                        <?php foreach ($materials as $material): ?>
                            <label class="flex items-center gap-3 text-sm cursor-pointer hover:text-accent transition-colors">
                                <input type="checkbox" name="mat[]" value="<?= esc($material->name) ?>" class="w-4 h-4 accent-primary" 
                                    <?= in_array($material->name, $selectedMaterials ?? []) ? 'checked' : '' ?> onchange="setActiveSection('materials'); this.form.submit()"> <?= esc($material->name) ?>
                            </label>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <div class="border-b border-border-light py-6">
                ...
                ?>
                <button type="button" class="w-full flex justify-between items-center mb-4 cursor-pointer focus:outline-none group" onclick="toggleFilter(this, 'dimensions')">
                    <span class="font-semibold text-[15px]">Dimensions (cm)</span>
                    <span class="text-xs transition-transform duration-200" style="transform: rotate(<?= $isDimOpen ? '0deg' : '-90deg' ?>);">▼</span>
                </button>
                <div class="px-1 <?= $isDimOpen ? '' : 'hidden' ?>">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative w-full h-10 mb-2">
                            <input type="range" name="width_min" id="width_min" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>" step="1" value="<?= $selectedWidthMin ?? 0 ?>" 
                                   class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updateWidthInputs()">
                            <input type="range" name="width_max" id="width_max" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>" step="1" value="<?= $selectedWidthMax ?? 500 ?>" 
                                   class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updateWidthInputs()">
                            
                            <div class="relative z-10 h-2 w-full bg-gray-200 rounded-lg">
                                <div class="absolute z-10 top-0 bottom-0 bg-accent rounded-lg" id="width-slider-track" style="left: 0%; right: 0%;"></div>
                                <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="width-thumb-min" onmousedown="startDrag(event, 'width', 'min')"></div>
                                <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="width-thumb-max" onmousedown="startDrag(event, 'width', 'max')"></div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between gap-2 text-sm">
                            <input type="number" id="input_width_min" value="<?= $selectedWidthMin ?? 0 ?>" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>"
                            ...
                            <input type="number" id="input_width_max" value="<?= $selectedWidthMax ?? 500 ?>" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>"
                                   class="w-20 px-1 py-1 border border-border-light rounded text-center focus:outline-none focus:border-accent text-xs"
                    ...
                    <div class="mb-2">
                        ...
                        <div class="relative w-full h-10 mb-2">
                            <input type="range" name="length_min" id="length_min" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>" step="1" value="<?= $selectedLengthMin ?? 0 ?>" 
                                   class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updateLengthInputs()">
                            <input type="range" name="length_max" id="length_max" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>" step="1" value="<?= $selectedLengthMax ?? 500 ?>" 
                                   class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updateLengthInputs()">
                            
                            <div class="relative z-10 h-2 w-full bg-gray-200 rounded-lg">
                                <div class="absolute z-10 top-0 bottom-0 bg-accent rounded-lg" id="length-slider-track" style="left: 0%; right: 0%;"></div>
                                <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="length-thumb-min" onmousedown="startDrag(event, 'length', 'min')"></div>
                                <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="length-thumb-max" onmousedown="startDrag(event, 'length', 'max')"></div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between gap-2 text-sm">
                            <input type="number" id="input_length_min" value="<?= $selectedLengthMin ?? 0 ?>" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>"
                                   class="w-20 px-1 py-1 border border-border-light rounded text-center focus:outline-none focus:border-accent text-xs"
                                   onchange="updateLengthSlider(this.value, 'min')">
                            <span class="text-gray-400">-</span>
                            <input type="number" id="input_length_max" value="<?= $selectedLengthMax ?? 500 ?>" min="<?= $dimMinBound ?? 0 ?>" max="<?= $dimMaxBound ?? 500 ?>"
                                   class="w-20 px-1 py-1 border border-border-light rounded text-center focus:outline-none focus:border-accent text-xs"
            ...
            <div class="border-b border-border-light py-6">
                ...
                <button type="button" class="w-full flex justify-between items-center mb-4 cursor-pointer focus:outline-none group" onclick="toggleFilter(this, 'sellers')">
                    <span class="font-semibold text-[15px]">Vendeurs</span>
                    <span class="text-xs transition-transform duration-200" style="transform: rotate(<?= $isSellerOpen ? '0deg' : '-90deg' ?>);">▼</span>
                </button>
                <div class="flex flex-col gap-3 <?= $isSellerOpen ? '' : 'hidden' ?>">
                    <?php if (!empty($sellers)): ?>
                        <?php foreach ($sellers as $seller): ?>
                            <label class="flex items-center gap-3 text-sm cursor-pointer hover:text-accent transition-colors">
                                <input type="checkbox" name="seller[]" value="<?= esc($seller->name) ?>" class="w-4 h-4 accent-primary" 
                                    <?= in_array($seller->name, $selectedSellers ?? []) ? 'checked' : '' ?> onchange="setActiveSection('sellers'); this.form.submit()"> <?= esc($seller->name) ?>
                            </label>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <div class="py-6">
                ...
                ?>
                <button type="button" class="w-full flex justify-between items-center mb-4 cursor-pointer focus:outline-none group" onclick="toggleFilter(this, 'price')">
                    <span class="font-semibold text-[15px]">Budget</span>
                    <span class="text-xs transition-transform duration-200" style="transform: rotate(<?= $isPriceOpen ? '0deg' : '-90deg' ?>);">▼</span>
                </button>
                <div class="px-1 <?= $isPriceOpen ? '' : 'hidden' ?>">
                    <div class="relative w-full h-10 mb-4">
                        <input type="range" name="price_min" id="price_min" min="0" max="5000" step="1" value="<?= $selectedPriceMin ?>" 
                               class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updatePriceInputs()">
                        <input type="range" name="price_max" id="price_max" min="0" max="5000" step="1" value="<?= $selectedPriceMax ?>" 
                               class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer" oninput="updatePriceInputs()">
                        
                        <div class="relative z-10 h-2 w-full bg-gray-200 rounded-lg">
                            <div class="absolute z-10 top-0 bottom-0 bg-accent rounded-lg" id="slider-track" style="left: 0%; right: 0%;"></div>
                            <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="thumb-min" onmousedown="startDrag(event, 'price', 'min')"></div>
                            <div class="absolute z-20 w-4 h-4 bg-white border-2 border-accent rounded-full -mt-1 -ml-2 cursor-pointer pointer-events-auto shadow-md hover:scale-110 transition-transform" id="thumb-max" onmousedown="startDrag(event, 'price', 'max')"></div>
                        </div>
                    </div>
            ...
            <button type="button" onclick="window.location.href='<?= base_url('catalog') ?>'" class="w-full bg-gray-100 text-primary py-3 rounded text-xs font-bold uppercase tracking-widest hover:bg-border-light transition-colors">
                Réinitialiser
            </button>
        </aside>
        ...
        <section>
            ...
            <div class="mb-8">
                <div class="relative max-w-xl">
                    ...
                    <button type="submit" class="absolute right-4 top-1/2 -translate-y-1/2 text-muted hover:text-accent transition-colors">
            ...
            <!-- Results Top Header -->
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 pb-4 border-b border-border-light">
                <div>
                    <nav class="text-[11px] uppercase tracking-wider text-text-muted mb-2">
                        <a href="<?= base_url('/') ?>" class="hover:text-accent">Accueil</a> / <span class="text-primary font-bold">Catalogue</span>
                    </nav>
                ...
                <div class="flex items-center gap-3 text-sm">
                    ...
                    <select name="sort" onchange="this.form.submit()" class="border border-border-light rounded px-3 py-2 bg-white focus:outline-none focus:border-accent cursor-pointer">
                        <option value="pertinence" <?= ($selectedSort ?? '') == 'pertinence' ? 'selected' : '' ?>>Pertinence</option>
                        <option value="prix_asc" <?= ($selectedSort ?? '') == 'prix_asc' ? 'selected' : '' ?>>Prix croissant</option>
                        <option value="prix_desc" <?= ($selectedSort ?? '') == 'prix_desc' ? 'selected' : '' ?>>Prix décroissant</option>
                        <option value="nouveautes" <?= ($selectedSort ?? '') == 'nouveautes' ? 'selected' : '' ?>>Nouveautés</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-6 gap-y-10">
                ...
                <?php else: ?>
                    <div class="col-span-full py-20 text-center">
                        <p class="text-text-muted italic text-lg font-serif">Aucun tapis ne correspond à votre recherche actuelle.</p>
                        <a href="<?= base_url('catalog') ?>" class="inline-block mt-4 text-accent font-bold border-b border-accent pb-1">Voir tout le catalogue</a>
                    </div>
                <?php endif; ?>
