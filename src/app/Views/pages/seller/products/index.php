...
<div class="flex items-center gap-4">
    ...
    <?php if (!empty($isSellerValidated)): ?>
        <a href="<?= base_url('seller/products/new') ?>"
            class="group flex items-center gap-2 px-5 py-2.5 bg-accent text-white rounded-xl hover:bg-accent-hover transition-all text-sm font-bold shadow-sm shadow-accent/20">
    ...
    <?php else: ?>
        <button disabled title="Votre compte doit être validé pour ajouter des produits"
            class="group flex items-center gap-2 px-5 py-2.5 bg-gray-200 text-gray-400 rounded-xl cursor-not-allowed text-sm font-bold">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
...
<form action="<?= current_url() ?>" method="get" class="mb-8">
    <div class="flex flex-col md:flex-row gap-4 items-center">
        ...
        <?php if (!empty($search) || !empty($status)): ?>
            <a href="<?= current_url() ?>"
                class="w-full md:w-auto text-center px-4 py-2.5 border border-border text-muted font-bold rounded-xl hover:bg-gray-50 transition-colors">
                Réinitialiser
            </a>
        <?php endif; ?>
    </div>
...
<?php if (empty($myProducts)): ?>
    <div class="bg-white rounded-2xl shadow-sm border border-border p-16 text-center">
        ...
        <p class="text-muted mb-8 max-w-md mx-auto">Commencez dès maintenant à vendre vos produits en les ajoutant à votre
            catalogue.</p>
        <a href="<?= base_url('seller/products/new') ?>"
            class="inline-flex items-center gap-2 text-accent hover:text-accent-hover font-bold hover:underline">
...
<?php else: ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <?php foreach ($myProducts as $p): ?>
            ...
            ?>
            <div
                class="group bg-white rounded-2xl shadow-sm border border-border overflow-hidden hover:shadow-lg hover:border-accent/30 transition-all duration-300 flex flex-col h-full relative">

                <!-- Image Area -->
                <div class="aspect-[4/3] bg-gray-100 relative overflow-hidden group-hover:opacity-90 transition-opacity">
                    <!-- Link to Edit -->
                    <a href="<?= base_url('seller/products/' . $p->id . '/edit') ?>" class="absolute inset-0 z-10"
                        aria-label="Modifier <?= esc($p->title) ?>"></a>
                    ...
                    <div class="absolute top-3 right-3 z-20 pointer-events-none">
                        <?php if ($isPending): ?>
                            <span
                                class="bg-amber-100/95 backdrop-blur-md text-amber-800 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5 border border-amber-200/50">
                                <span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span> En attente
                            </span>
                        <?php elseif ($isApproved): ?>
                            <span
                                class="bg-green-100/95 backdrop-blur-md text-green-800 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5 border border-green-200/50">
                                <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> Validé
                            </span>
                        <?php else: ?>
                            <span
                                class="bg-red-100/95 backdrop-blur-md text-red-800 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5 border border-red-200/50">
                ...
                <div class="p-5 flex-1 flex flex-col">
                    <div class="flex justify-between items-start mb-2">
                        <h3 class="font-bold text-lg text-primary group-hover:text-accent transition-colors line-clamp-1"
                            title="<?= esc($p->title) ?>">
                            <?= esc($p->title) ?>
                        </h3>
                    </div>
                    ...
                    <div class="mt-auto space-y-3">
                        <div
                            class="flex items-center justify-between text-sm py-3 border-t border-dashed border-border text-muted">
                            <span>Stock disponible</span>
                            <span
                                class="font-mono font-bold <?= ($p->stock_available < 5) ? 'text-red-500' : 'text-primary' ?>">
                                <?= $p->stock_available ?>
                            </span>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 border-t border-border px-5 py-3 flex items-center justify-between z-20">
                    <a href="<?= base_url('seller/products/' . $p->id . '/edit') ?>"
                        class="text-xs font-bold text-muted hover:text-accent uppercase tracking-wide transition-colors flex items-center gap-1">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <form action="<?= base_url('seller/products/' . $p->id) ?>" method="post"
                        onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce produit ? Cette action est irréversible.');"
                        class="inline">
                        <?= csrf_field() ?>
                        <input type="hidden" name="_method" value="DELETE">
                        <button type="submit"
                            class="text-gray-400 hover:text-red-600 transition-colors p-1.5 hover:bg-red-50 rounded-full"
                            title="Supprimer">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
