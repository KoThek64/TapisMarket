...
<div
    class="hidden md:flex items-center gap-4 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-xl shadow-sm border border-border">
    ...
    <div class="text-right">
        ...
        <span class="block text-xl font-serif font-bold text-accent"><?= number_format($stats['turnover'] ?? 0, 2) ?>
            €</span>
    </div>
</div>
<?= $this->endSection() ?>
...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    <div
        class="flex flex-col sm:flex-row gap-4 justify-between items-end sm:items-center bg-white p-2 pr-4 rounded-2xl shadow-sm border border-border">
            ...
            <?php foreach ($tabs as $status => $config):
                ...
                <a href="<?= base_url('seller/orders?status=' . $status) ?>" class="<?= $baseClass ?> <?= $style ?>">
                    ...
                    <?php if ($status !== 'ALL' && $hasOrders): ?>
                        <span
                            class="bg-white/20 px-1.5 py-0.5 rounded text-[10px] ml-1 shadow-sm font-extrabold"><?= $count ?></span>
                    <?php endif; ?>
                    ...
                    <?php if ($status !== 'ALL' && $status !== 'DELIVERED' && $status !== 'CANCELLED' && !$isActive && $hasOrders): ?>
                        <span class="flex h-2.5 w-2.5 absolute top-0 right-0 -mt-1 -mr-1">
                            <?php if (in_array($status, ['PAID', 'PREPARING'])): ?>
                                <span
                                    class="animate-ping absolute inline-flex h-full w-full rounded-full bg-<?= $color ?>-400 opacity-75"></span>
                            <?php endif; ?>
                            <span
                                class="relative inline-flex rounded-full h-2.5 w-2.5 bg-<?= $color ?>-500 border border-white"></span>
                        </span>
                    <?php endif; ?>
                </a>
            <?php endforeach; ?>
        ...
        <?php if ($currentStatus !== 'ALL'): ?>
            <a href="<?= base_url('seller/orders') ?>"
                class="text-sm text-muted hover:text-primary flex items-center gap-1 transition-colors px-2">
    ...
    <?php else: ?>
        <div class="space-y-6">
            <?php foreach ($orders as $orderData):
                ...
                ?>
                <div
                    class="bg-white rounded-2xl shadow-sm border border-border overflow-hidden transition-all duration-200 hover:shadow-md">
                    <div class="p-5 flex flex-col md:flex-row justify-between gap-4">
                        <div class="flex items-start gap-4">
                            <div
                                class="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center border border-border text-primary font-bold shadow-sm shrink-0">
                                #
                            </div>
                            <div class="space-y-1">
                                <div class="flex items-center gap-3">
                                    ...
                                    ?>
                                    <span
                                        class="<?= $badgeColor ?> px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide leading-none">
                                        <?= esc($displayStatus) ?>
                                    </span>
                                </div>
                                ...
                                <p class="text-sm text-muted">
                                    <span class="inline-flex items-center gap-1">
                                        ...
                                        <?= date('d/m/Y', strtotime($info->order_date)) ?> à
                                        <?= date('H:i', strtotime($info->order_date)) ?>
                                    </span>
                        ...
                        <div
                            class="flex flex-col md:items-end gap-2 md:text-right border-t md:border-t-0 border-dashed border-gray-200 pt-4 md:pt-0">
                            <div>
                            ...
                            <div class="flex flex-wrap gap-2 mt-1">
                                <?php if ($info->status === 'PAID'): ?>
                                    <form action="<?= base_url('seller/orders/update-status/' . $info->order_id) ?>" method="POST">
                                        ...
                                        <button type="submit"
                                            class="inline-flex items-center gap-1 text-xs bg-amber-50 text-amber-700 hover:bg-amber-100 font-medium px-3 py-1.5 rounded-lg border border-amber-200 transition-colors w-full md:w-auto justify-center">
                                            <span>Préparer</span>
                                ...
                                <?php if ($info->status === 'PREPARING'): ?>
                                    <form action="<?= base_url('seller/orders/update-status/' . $info->order_id) ?>" method="POST">
                                        ...
                                        <button type="submit"
                                            class="inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium px-3 py-1.5 rounded-lg border border-blue-200 transition-colors w-full md:w-auto justify-center">
                                            <span>Expédier</span>
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <?php if ($info->status === 'SHIPPED'): ?>
                                    <form action="<?= base_url('seller/orders/update-status/' . $info->order_id) ?>" method="POST">
                                        ...
                                        <button type="submit"
                                            class="inline-flex items-center gap-1 text-xs bg-green-50 text-green-700 hover:bg-green-100 font-medium px-3 py-1.5 rounded-lg border border-green-200 transition-colors w-full md:w-auto justify-center">
                    ...
                    <details class="group border-t border-border bg-gray-50/30">
                        <summary
                            class="flex justify-between items-center p-4 cursor-pointer hover:bg-gray-50 transition-colors select-none">
                            ...
                            <span
                                class="text-gray-400 group-open:text-accent group-open:rotate-180 transition-all duration-300">
                                        ...
                                        <tbody class="divide-y divide-border">
                                            <?php foreach ($items as $item): ?>
                                                <tr class="hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td class="px-4 py-3 text-center text-primary font-medium">
                                                        <?= $item->quantity ?></td>
                                ...
                                <div class="bg-cream px-6 py-5 text-sm border-t border-border flex flex-col md:flex-row gap-6">
                                    ...
                                    <div class="flex-1 flex gap-4">
                                        <div class="shrink-0 pt-1">
                                            <div
                                                class="p-2 bg-white rounded-full text-accent shadow-sm border border-accent/20">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div>
                                            <span
                                                class="block text-xs font-bold text-muted uppercase tracking-wider mb-1">Adresse
                                                de livraison</span>
                                            <div class="font-medium text-primary text-base">
                                                <?= esc($info->delivery_street ?? '') ?>
                                            </div>
                                            <div class="text-primary">
                                                <?= esc($info->delivery_postal_code ?? '') ?>
                                                <?= esc($info->delivery_city ?? '') ?>
                                            </div>
                                    ...
                                    <?php if (!empty($info->customer_phone)): ?>
                                        <div class="flex-1 md:border-l md:border-dashed md:border-gray-300 md:pl-6 flex gap-4">
                                            ...
                                            <div>
                                                <span
                                                    class="block text-xs font-bold text-muted uppercase tracking-wider mb-1">Contact
                                                    Client</span>
