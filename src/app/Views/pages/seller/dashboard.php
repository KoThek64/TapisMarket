...
<a href="<?= site_url('/') ?>"
    class="group flex items-center gap-2 px-5 py-2.5 bg-white border border-border rounded-full hover:border-accent hover:text-accent transition-all text-sm font-bold shadow-sm">
    ...
    <div
        ...
        <div class="flex justify-between items-start">
            <div>
                ...
                <h3 class="font-serif text-3xl font-bold text-primary group-hover:text-amber-500 transition-colors">
                    <?= number_format($averageRating, 1) ?> <span class="text-lg text-muted">/5</span></h3>
            </div>
        </div>
    </div>
    ...
    <div
        ...
        <div class="flex justify-between items-start">
            <div>
                ...
                <h3 class="font-serif text-3xl font-bold text-primary group-hover:text-purple-600 transition-colors">
                    <?= $pendingProducts ?></h3>
                <?php if ($pendingProducts > 0): ?>
                    <a href="<?= base_url('seller/products') ?>" class="text-xs text-accent mt-2 block hover:underline">Voir
                        les produits</a>
                <?php endif; ?>
            </div>
        </div>
                        ...
                        <?php foreach ($recentSales as $sale): ?>
                            <tr class="hover:bg-cream transition-colors group">
                                ...
                                <td class="px-8 py-4 text-right">
                                    <span
                                        class="font-bold text-primary"><?= number_format($sale->unit_price * $sale->quantity, 2) ?>
                                        €</span>
                                </td>
        ...
        <div class="p-8">
            <div class="grid grid-cols-2 gap-4">
                <a href="<?= base_url('seller/products/new') ?>"
                    class="bg-cream p-6 rounded-xl border border-border hover:border-accent hover:shadow-md transition-all flex flex-col items-center justify-center gap-3 group h-32">
                    <div
                        class="w-12 h-12 bg-accent/10 rounded-full flex items-center justify-center text-accent group-hover:bg-accent group-hover:text-white transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4">
                            </path>
                        </svg>
                    </div>
                    <span class="font-bold text-sm text-primary group-hover:text-accent transition-colors">Ajouter un
                        produit</span>
                </a>
                <a href="<?= base_url('seller/products') ?>"
                    class="bg-cream p-6 rounded-xl border border-border hover:border-blue-500 hover:shadow-md transition-all flex flex-col items-center justify-center gap-3 group h-32">
                    <div
                        class="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 6h16M4 10h16M4 14h16M4 18h16"></path>
                        </svg>
                    </div>
                    <span class="font-bold text-sm text-primary group-hover:text-blue-600 transition-colors">Gérer le
                        stock</span>
                </a>
            </div>
        </div>
