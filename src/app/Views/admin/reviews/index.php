    ...
    <div class="px-8 py-6 border-b border-border bg-gray-50/30 flex justify-between items-center">
        <div class="flex items-center gap-4">
            ...
            <?php if ($criticalCount > 0): ?>
                <a href="<?= site_url('admin/reviews?filter=critical') ?>"
                    class="bg-red-100 text-red-600 text-[10px] px-3 py-1 rounded-full uppercase font-bold tracking-tighter border border-red-200 hover:bg-red-200 transition">
        ...
        <form method="get" action="<?= site_url('admin/reviews') ?>">
            <div class="relative group">
                <div
                    class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-muted group-hover:text-accent transition-colors">
                ...
                <select name="filter" onchange="this.form.submit()"
                    ...
                    <option value="critical" <?= ($currentFilter === 'critical') ? 'selected' : '' ?>> Note basse (≤ 2★)
                    </option>
                </select>

                <div
                    class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-muted group-hover:text-primary transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr class="hover:bg-cream transition-colors group">
                        ...
                        <td class="px-8 py-5">
                            <div class="relative group w-fit max-w-full">
                                <p class="text-muted italic text-xs leading-relaxed line-clamp-3">"<?= esc($a->comment) ?>"
                                </p>
                                <div
                                    class="absolute top-full left-0 mt-2 w-80 bg-gray-900 text-white text-xs p-4 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none border border-gray-700">
                                    <div class="absolute bottom-full left-4 border-8 border-transparent border-b-gray-900">
                                    </div>
                                    <p class="font-normal not-italic leading-relaxed relative z-10"><?= esc($a->comment) ?>
                                    </p>
                                </div>
                            </div>
                            <?php if ($a->moderation_status === REVIEW_REFUSED): ?>
                                <span
                                    class="inline-block mt-2 text-[10px] font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded border border-red-200">Masqué</span>
                            <?php endif; ?>
                        </td>

                        <td class="px-8 py-5">
                            <div class="text-sm font-medium text-primary"><?= esc($a->firstname) ?>     <?= esc($a->lastname) ?>
                            </div>
                        ...
                        <td class="px-8 py-5 text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <a href="javascript:void(0)"
                                    onclick="openDeleteModal('<?= site_url('admin/reviews/' . $a->id) ?>')"
