...
<?= $this->section('content') ?>
    ...
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        <!-- Sorting Toolbar -->
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-border">
                ...
                <?php foreach($reviews as $review): ?>
                    <div class="bg-white rounded-2xl shadow-sm border border-border p-6 hover:shadow-md transition-shadow">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-amber-50 rounded-full flex items-center justify-center text-amber-600 font-bold border border-amber-100 shadow-sm">
                                    <?= substr($review->firstname, 0, 1) ?>
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h4 class="font-bold text-primary"><?= esc($review->firstname . ' ' . $review->lastname) ?></h4>
                                        <?php if(isset($review->orders_count)): ?>
                                            <?php if($review->orders_count > 0): ?>
                                                <span class="inline-flex items-center justify-center bg-blue-50 text-blue-700 text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-100" title="Ce client a passé <?= $review->orders_count ?> commande(s) chez vous">
                                                    <?= $review->orders_count ?> CMD
                                                </span>
                                            <?php else: ?>
                                                <span class="inline-flex items-center justify-center bg-gray-100 text-gray-500 text-[10px] font-bold px-1.5 py-0.5 rounded border border-gray-200" title="Aucune commande validée trouvée pour ce client">
                                                    Non vérifié
                                                </span>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    ...
                                    <div class="flex text-amber-500 text-xs gap-0.5 mt-0.5">
                                        <?php for($i=1; $i<=5; $i++): ?>
                                            <svg class="w-4 h-4 <?= $i <= $review->rating ? 'fill-current' : 'text-gray-200 fill-current' ?>" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.038 3.181a1.002 1.002 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1.001 1.001 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1.001 1.001 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1.001 1.001 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1.002 1.002 0 00.951-.69l1.037-3.181z"/></svg>
                                        <?php endfor; ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Product Link -->
                        <div class="mb-4">
                            <a href="<?= base_url('seller/products/' . $review->product_id . '/edit') ?>" class="text-xs font-medium text-accent hover:underline flex items-center gap-1">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-50">
                             <?php if ($review->moderation_status === 'PUBLISHED'): ?>
                                <span class="badge flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> Publié
                                </span>
                             <?php else: ?>
                                <span class="badge flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-red-50 text-red-700">
