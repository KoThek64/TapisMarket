...
<?= $this->section('header_content') ?>
    <a href="<?= site_url('seller/shop/edit') ?>" class="px-4 py-2 rounded-xl bg-accent text-white hover:bg-accent-hover transition flex items-center gap-2 shadow-sm font-bold text-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-border p-6 space-y-4">
        <div class="flex items-center justify-between">
            ...
            <?php if(isset($shop->status)):
                $status = $shop->status;
                $label = null; $cls = '';
                if (defined('SELLER_PENDING') && $status === SELLER_PENDING) { $label='En attente'; $cls='bg-yellow-100 text-yellow-800'; }
                if (defined('SELLER_VALIDATED') && $status === SELLER_VALIDATED) { $label='Validé'; $cls='bg-green-100 text-green-800'; }
                if (defined('SELLER_REFUSED') && $status === SELLER_REFUSED) { $label='Refusé'; $cls='bg-red-100 text-red-800'; }
                if (defined('SELLER_SUSPENDED') && $status === SELLER_SUSPENDED) { $label='Suspendu'; $cls='bg-orange-100 text-orange-800'; }
                if ($label): ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="text-lg font-mono"><?= esc(method_exists($shop, 'getFormattedSiret') ? $shop->getFormattedSiret() : ($shop->siret ?? '')) ?></p>
            </div>
